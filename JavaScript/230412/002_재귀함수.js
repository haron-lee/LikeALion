// * 재귀함수의 중요도 ⭐️
// 다이나믹 프로그래밍, 메모라이징 기법 등을 사용하는 순간이 오게 되면 재귀함수의 중요도가 ⭐️⭐️

// ! 재귀함수 => 내가 나를 호출하는 것
// 반복문으로 반복할 수 있는 것은 재귀함수로 만들 수 있다.
// 재귀함수로 만들 수 있는 것은 반복문으로도 만들 수 있다.
// 실무에서는 반복문 사용하길 권장. 알고리즘 문제에서는 간혹 재귀를 호출해야 할 일이 있다. (팰린드롬: 오디오, 기러기, 스위스, 토마토)
// 병합정렬, 퀵정렬
...
// * 누적합
// 이렇게 일일이 순회하지 않고 구할 수 있는 값이기 때문에 2개다 비효율적인 코드이다. 특히 소수를 구하는 문제 같은 곳에서 2중 for문 사용하는 것도 매우 비효율이다.
result = 1;
...
function reverse(txt) {
  if (txt.length <= 1) return txt[0] + reverse(txt.slice(1));
}
reverse("hello world");

function reverse(txt) {
  if (txt.length <= 1) return reverse(txt.slice(1) + txt[0]);
}
reverse("hello world");
